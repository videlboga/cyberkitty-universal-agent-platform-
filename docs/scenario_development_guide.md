...
Плагин `SchedulingPlugin` позволяет планировать запуск других сценариев через определенное время.

#### 4.10.1. `schedule_scenario_run`

-   **Назначение**: Запланировать запуск другого сценария (агента) через указанное количество секунд.
-   **Параметры (`params`)**:
    -   `run_in_seconds` (число, обязательное): Через сколько секунд от текущего момента запустить сценарий.
    -   `scenario_id_to_run` (строка, обязательное): ID или имя сценария (агента), который должен быть запущен. Планировщик использует это как `agent_id`.
...
-   **Пример**:
    ```json
    {
      ...
      "params": {
        ...
        "scenario_id_to_run": "user_follow_up_scenario",
        "context_to_pass": {
...
-   **Текущее состояние**: **Нет стандартного типа шага** `schedule_task` для создания отложенных задач или напоминаний (например, "рассказывает мне ллм шутку через 2 минуты"). `SchedulerService` существует в системе, но плагин, который предоставлял бы к нему доступ из сценариев (`LearningPlanPlugin`), был удален.
-   **Влияние**: Реализация шага "Запускается через 2 минуты..." в вашем тестовом сценарии невозможна без доработки.
-   **Возможные пути решения**:
    1.  Восстановление или создание нового плагина, который предоставляет интерфейс к `SchedulerService` через шаг сценария.

**Вывод**: Для полной реализации вашего тестового сценария **только средствами существующих типов шагов сценариев** потребуется предварительная доработка системы или создание/модификация плагинов для обеспечения функциональности работы с БД и планирования задач. Части сценария, связанные с опросом, вызовом LLM и взаимодействием через Telegram, могут быть реализованы с текущими возможностями. 