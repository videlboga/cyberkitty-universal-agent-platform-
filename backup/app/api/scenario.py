...
async def create_scenario(scenario: Scenario):
    try:
        ...
        # Получение полного объекта сценария
        try:
            created_full = await scenario_repo.get_by_id(created.id)
            return created_full
        except Exception as e:
            # Обработка ошибки получения полного сценария
            logger.error(f"Ошибка при получении созданного сценария: {e}")
            # Сценарий уже создан, поэтому возвращаем базовую версию
            return created
