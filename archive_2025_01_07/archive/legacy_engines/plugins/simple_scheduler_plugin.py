...
class SimpleSchedulerPlugin(BasePlugin):
    ...
    async def create_task(self, step: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"📅 Создана задача {task_id}: сценарий {scenario_id} через {delay_minutes} мин с контекстом {task_context}")
    ...
    async def cancel_task(self, step: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Отменяет задачу или задачи."""
        try:
            # Новые параметры для массовой отмены
            cancel_all = self.get_param(step, "cancel_all", default=False)
            user_id = self.get_param(step, "user_id")
            scenario_id = self.get_param(step, "scenario_id")
            
            # Старый параметр для отмены одной задачи
            task_id = self.get_param(step, "task_id")
            output_var = self.get_param(step, "output_var")
            
            cancelled_count = 0
            
            if cancel_all and user_id:
                # Отменяем все задачи пользователя (и опционально сценария)
                tasks_to_remove = []
                
                for tid, task in self.tasks.items():
                    if task.user_id == user_id and not task.executed:
                        # Если указан scenario_id, проверяем его тоже
                        if not scenario_id or task.scenario_id == scenario_id:
                ...
                for tid in tasks_to_remove:
                    del self.tasks[tid]
                    cancelled_count += 1
                    logger.info(f"🗑️ Задача {tid} отменена")
                
                if cancelled_count > 0:
                    result = {
                        "success": True, 
                        "message": f"Отменено {cancelled_count} задач",
                        "cancelled_count": cancelled_count
                    }
                    logger.info(f"🗑️ Отменено {cancelled_count} задач для пользователя {user_id}")
                else:
                    result = {
                        "success": False, 
                        "error": "Нет задач для отмены",
                        "cancelled_count": 0
                    }
            
            elif task_id:
                # Отменяем одну конкретную задачу (старая логика)
                if task_id not in self.tasks:
                    result = {"success": False, "error": "Задача не найдена"}
                else:
                    task = self.tasks[task_id]
                ...
                result = {"success": False, "error": "Не указан task_id или параметры для массовой отмены"}
